...
from platform_api import (AsyncTokenManager, AsyncProxyPool, fetch_tasks_from_api)

from palt_api import fetch_domain_by_task_id, get_task_info
...
async def worker(worker_id: int, pool: BrowserPool):
    while True:
        ...
        try:
            ...
            worker_info_list = await get_task_info(atm, session)
            if not worker_info_list:
                print("not task sleep 30s")
                await asyncio.sleep(30)

            worker_info = worker_info_list[0]
            worker_id = worker_info["id"]

            tasks = await fetch_domain_by_task_id(atm, session, worker_id)
            ...
            if link_session:
                try:
                    ...
                    logger.warning(f"关闭 link_session 失败: {e}")
