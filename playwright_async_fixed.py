...
def make_response_handler(task_id, params, aggregated_data):
    ...
    async def handle_response(response):
        ...
        if 'https://www.google.com/search' in url:
            ...
            logger.info(f"Worker ID: {params.worker_id} 捕获到搜索请求: {url}")
            ...
            try:
                ...
                logger.info(
                    f"Worker ID: {params.worker_id} related search num: {len(related_search)} {related_search[:3]}...")
                aggregated_data['related_search'].extend(related_search)

                # 收集 related_items
                related_items = await get_related_items(body)
                logger.info(
                    f"Worker ID: {params.worker_id} related item num: {len(related_items)} {related_items[:3]}...")
...
async def search_single_keyword(browser, keyword_item, params, max_retries=2):
    ...
    for attempt in range(max_retries):
        try:
            ...
            # 打开 Google 图片搜索
            logger.info(
                f"Worker ID: {params.worker_id} [{keyword}] 正在打开 Google 图片搜索页面 (尝试 {attempt + 1}/{max_retries})")
            ...
            # 搜索关键词
            logger.info(f"Worker ID: {params.worker_id} [{keyword}] 开始输入关键词")
            await asyncio.wait_for(
            ...
            # 平滑滚动
            logger.info(f"Worker ID: {params.worker_id} [{keyword}] 开始滚动页面")
            await asyncio.wait_for(
                human_scroll(page, 6),
                timeout=60.0
            )
            ...
            if aggregated_data['new_datas']:
                logger.info(
                    f"Worker ID: {params.worker_id} [{keyword}] 开始处理聚合数据，共 {len(aggregated_data['new_datas'])} 条")
                ...
                logger.info(f"Worker ID: {params.worker_id} [{keyword}] 数据处理完成")

            return True

        except asyncio.TimeoutError:
            logger.error(f"Worker ID: {params.worker_id} [{keyword}] 搜索超时 (尝试 {attempt + 1}/{max_retries})")
            if page:
                try:
                    await page.close()
                except:
            ...
            if attempt < max_retries - 1:
                ...
                logger.error(f"Worker ID: {params.worker_id} [{keyword}] 已达最大重试次数，跳过")
                return False

        except PlaywrightTimeout as e:
            logger.error(
                f"Worker ID: {params.worker_id} [{keyword}] Playwright 超时 (尝试 {attempt + 1}/{max_retries}): {e}")
            if page:
                try:
                    await page.close()
                except:
            ...
            logger.exception(
                f"Worker ID: {params.worker_id} [{keyword}] 搜索异常 (尝试 {attempt + 1}/{max_retries}): {e}")
            if page:
                try:
                    await page.close()
                except:
...
async def search_keyword_batch(params):
    ...
    try:
        ...
        # 初始化浏览器，带超时
        logger.info(f"Worker ID: {params.worker_id} 初始化浏览器，代理: {params.proxies['server']}")
        await asyncio.wait_for(
            browser.initialize(),
            timeout=60.0
        )
        ...
        for keyword_item_str in params.tasks:
            ...
            logger.info(f"Worker ID: {params.worker_id} 开始搜索: {keyword_item['name']}")
        ...
        logger.info(f"Worker ID: {params.worker_id} 批次完成 - 成功: {success_count}, 失败: {fail_count}")

    except asyncio.TimeoutError:
        logger.error(f"Worker ID: {params.worker_id} 浏览器初始化超时")
        raise
    except Exception as e:
        logger.exception(f"Worker ID: {params.worker_id} 批量搜索异常: {e}")
        raise
    finally:
        if browser:
            logger.info(f"Worker ID: {params.worker_id} 关闭浏览器")
